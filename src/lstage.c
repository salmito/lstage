...
void
stackDump (lua_State * L, const char *text)
{
	int i;
...
void
tableDump (lua_State * L, int idx, const char *text)
{
...
static int
lstage_setmetatable (lua_State * L)
{
	lua_pushvalue (L, 2);
...
int
lstage_getmetatable (lua_State * L)
{
		  ...
		  if (!lua_getmetatable (L, 1))
		    {
			    lua_pushnil (L);
			    lua_pushliteral (L,
					     "Metatable not found for the provided value");
			    return 2;
		    }
	  }
	return 1;
...
static void
lstage_require (lua_State * L, const char *lib, lua_CFunction func)
{
