...
pool_t
lstage_topool (lua_State * L, int i)
{
...
void
lstage_buildpool (lua_State * L, pool_t t)
{
	...
	if (lua_isnil (L, -1))
	  {
		  ...
		  luaL_loadstring (L,
				   "local ptr=(...):ptr() return function() return require'lstage.pool'.get(ptr) end");
...
luaopen_lstage_pool (lua_State * L)
{
	lua_newtable (L);
	lua_newtable (L);
	luaL_loadstring (L,
			 "return function() return require'lstage.pool' end");
