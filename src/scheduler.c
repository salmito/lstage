...
thread_t
lstage_tothread (lua_State * L, int i)
{
	...
	if (lua_isnil (L, -1))
	  {
		  ...
		  luaL_loadstring (L,
				   "local th=(...):ptr() return function() return require'lstage.scheduler'.build(th) end");
		  lua_setfield (L, -2, "__wrap");
	  }
}

static void
thread_resume_instance (instance_t i)
{
...
int
lstage_newthread (lua_State * L, pool_t pool)
{
...
void
lstage_pushinstance (instance_t i)
{
...
luaopen_lstage_scheduler (lua_State * L)
{
...
//      if(!ready_queue) ready_queue=lstage_lfqueue_new();
	...
	lua_newtable (L);
	luaL_loadstring (L,
			 "return function() return require'lstage.scheduler' end");
