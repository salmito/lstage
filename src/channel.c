...
channel_t
lstage_tochannel (lua_State * L, int i)
{
	...
	if (lua_isnil (L, -1))
	  {
		  ...
		  luaL_loadstring (L,
				   "local id=(...):id() return function() return require'lstage.channel'.__get(id) end");
		  lua_setfield (L, -2, "__wrap");
	  }
}

static void
channel_build (lua_State * L, channel_t t)
{
...
luaopen_lstage_channel (lua_State * L)
{
	lua_newtable (L);
	lua_newtable (L);
	luaL_loadstring (L,
			 "return function() return require'lstage.channel' end");
