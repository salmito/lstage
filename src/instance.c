...
void
lstage_initinstance (instance_t i)
{
...
void
lstage_putinstance (instance_t i)
{
	event_t ev = NULL;
	...
	_DEBUG ("instance is now I_IDLE %p lua_State %p (%u)\n", i, i->L,
		lstage_lfqueue_size (i->stage->instances));
...
void
lstage_destroyinstance (instance_t i)
{
	lua_close (i->L);
	if (i->ev)
